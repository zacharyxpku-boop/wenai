...
export default function ContentMarketingPackWorkspace() {
  const [input, setInput] = useState<ContentMarketingPackInput>({
    category: 'home',
    sku: 'Drawer organizer 10 SKU launch batch',
    platform: 'both',
    benchmarkLinks: '',
    brandVoice: 'clean, credible, not exaggerated',
    campaignGoal: 'validate TikTok and Instagram hooks before scaling the SKU batch',
  });
        ...
        <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            ...
            <h1 className="text-balance font-[family-name:var(--font-outfit)] text-3xl font-bold text-text-primary md:text-4xl">
              TikTok / Instagram benchmark 到内容营销交付包
            </h1>
            <p className="mt-3 text-pretty text-[14px] leading-relaxed text-text-secondary">
              不是泛营销灵感页。这里把 SKU、平台、benchmark 和类目护栏压成 hook matrix、slideshow brief、reel brief 和发布复盘报告。
            </p>
          </div>
          <Link
            href="/poc"
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <section className="rounded-lg border border-border-subtle bg-bg-surface/35 p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <div className="text-[10px] font-mono uppercase tracking-wider text-accent">Input</div>
            ...
            <div className="space-y-3">
              ...
              <Field label="TikTok / Instagram benchmark 链接或账号">
                <textarea
                  value={input.benchmarkLinks}
                  onChange={event => setInput(prev => ({ ...prev, benchmarkLinks: event.target.value }))}
                  rows={4}
                  placeholder="没有也能生成 hypothesis pack，但发布前必须补 benchmark。"
                  className="w-full resize-none rounded-md border border-border-default bg-bg-root px-3 py-2 text-[13px]"
                />
              </Field>

              <Field label="Campaign 目标">
                <textarea
            ...
            <div className="mt-4 rounded-md border border-accent/30 bg-accent/10 p-3">
              <div className="mb-1 text-[10px] font-mono uppercase tracking-wider text-accent">Standard pack handoff</div>
              <p className="text-[12px] leading-relaxed text-text-secondary">
                当前交付包可以继续进入标准包工作台，形成客户可复制的 SOP 标品。
              </p>
              <Link
          ...
          <section className="rounded-lg border border-border-subtle bg-bg-surface/20 p-5">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <span className="rounded border border-accent/35 px-2 py-1 text-[10px] font-mono text-accent">
                benchmark → hook → brief → report
              </span>
            </div>

            <div className="space-y-4">
              <Panel title="Hook matrix">
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  {pack.hookMatrix.map(row => (
                    <div key={row.angle} className="rounded-md border border-border-subtle bg-bg-root/45 p-3">
                      ...
                      <p className="mt-2 text-[11px] leading-relaxed text-text-secondary">First frame: {row.firstFrame}</p>
                      <p className="mt-1 text-[11px] leading-relaxed text-text-secondary">Proof: {row.proofNeeded}</p>
                    </div>
                  ))}
                </div>
              </Panel>
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <Panel title="Slideshow / Reel brief">
                  <List items={[...pack.slideshowBrief, ...pack.reelBrief]} />
                </Panel>
                <Panel title="Publishing report">
                  <List items={pack.publishingReport} />
                </Panel>
              </div>

              <Panel title="Redlines">
